...
business_cycle_with_noise = business_cycle_t.(1:(365*2)) .^ rand(Truncated(Poisson(5), 1, 5), 365*2)
business_cycle_default_risk = (rand(Uniform(0.1, 0.5)) .* business_cycle_with_noise)
# plot(1:(365*2), business_cycle_risk)

business_cycle_df = DataFrame("application_date" => 1:(365*2), "business_cycle_default_risk" => business_cycle_default_risk)
individuals_df = DataFrame(sample(individual_attributes(), NUTS(1000, 0.65), n))
individuals_df[!, :application_date] .= round.(individuals_df[!, :application_date], digits=0)
individuals_df
